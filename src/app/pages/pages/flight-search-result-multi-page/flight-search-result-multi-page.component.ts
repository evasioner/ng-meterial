...
export class FlightSearchResultMultiPageComponent extends BasePageComponent implements OnInit, OnDestroy {
    ...
    compareDetailVisible: Boolean = false;
            ...
            this.flightListSubject$
                .pipe(
                    ...
                    distinctUntilChanged(
                        (before: any, now: any) => {
                            console.info('[API 호출 | 항공편 리스트(오는편) >]', before, now);
                    ...
                    switchMap(
                        req => {
                            ...
                            return this.apiflightSvc.POST_FLIGHT_LIST_ITINERARY(this.resolveData.rq)
                                .pipe(
                                    finalize(() => {
                                        console.log('POST_FLIGHT_LIST_ITINERARY completed.');
    ...
    private subscribeInit() {
        this.subscriptionList.push(
            this.store.select(
                ...
                .subscribe(
                    (resp: any) => {
                        console.log('asjdf;lkjasdfkl;jasldfkjla;ksdfj;laskfjdla;s 정말 이게 도는건가? ');
                        if (resp && this.resolveData.rs) {
                            console.info('[flight-search-result-come > flight-selected-item subscribe]', resp);
                            this.selectedItem = _.cloneDeep(resp.option);
                            this.onSchedule();
                        }
                    },
                    (error: any) => {
                        console.info('[flight-search-result-come > flight-selected-item subscribe error]', error);
                    }
                )
        );
    }
    ...
    */
    ...
    goToResultPage() {
        ...
        this.subscriptionList.push(
            this.bsModalService.onHidden
                .subscribe(
                    () => {
                        console.log('[flight-search-result-multi > flight-modal-schedule] ', bsModalRef.content.dataModel.returnData);

                        // 페이지 이동 시 모달용 store는 초기화 꼭 해줄것
                        if (bsModalRef.content.dataModel.returnData) {
                            if (!bsModalRef.content.dataModel.returnData.complete) {
                                this.router.navigate([bsModalRef.content.dataModel.returnData.path], { relativeTo: this.route });
                            } else {
                                const promotionState: any = {
                                    promotionRq: this.promotionRqCreate(this.selectedItem),
                                    cabinClassTxt: this.vmInfo.travelerStore.cabinClassTxt
                                };

                                console.log('flight-search-result-com > initialState promotionRq >>', promotionState);
                                this.bsModalService.show(FlightModalPaymentMethodComponent, { ...promotionState, ...this.configInfo });
                                this.subscriptionList.push(
                                    this.bsModalService.onHidden
                                        .subscribe(
                                            () => {
    ...
    onSchedule() {
        console.info('[항공편 선택 | 오는편]', this.selectedItem);
        ...
        console.info('[flight-search-result-come flightStepRQ >>]', this.flightStepRQ);
        // 임시 방편 rs 없으면 실행 안 시킴
        if (this.resolveData.rs) {
            this.goToResultPage();
        }
    }
    ...
    public onSearchOutEvt(data: any) {
        console.log('onSearchOutEvt >', data);

        // 재 검색(필터 적용)
        this.location.replaceState(`/flight-search-result-come/${encodeURIComponent(qs.stringify(data))}`);
