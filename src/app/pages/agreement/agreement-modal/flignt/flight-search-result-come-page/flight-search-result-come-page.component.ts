...
export class FlightSearchResultComePageComponent extends BasePageComponent implements OnInit, OnDestroy {
    ...
    */
    ...
    resConvert($Request: any) {
        console.info('[queryString to Integer Convert 시작]', $Request);
    ...
    goToResultPage() {
        ...
        this.subscriptionList.push(
            bsModalRef.content.onClose
                .subscribe(
                    (flag: boolean) => {
                        console.log('[flight-search-result-come > flight-modal-schedule] ', flag);
                            ...
                            if (!bsModalRef.content.dataModel.returnData.complete) {
                                ...
                                this.subscriptionList.push(
                                    payModalRef.content.onClose
                                        .subscribe(
                                            (item: boolean) => {
                                                console.log('[flight-search-result-come > flight-modal-payment close] : ', item);
                                                this.closeAllModals();
                                            }
                                        )
                                );
    ...
    onSchedule() {
        ...
        if (_.has(this.flightStepRQ.condition.filter, 'itineraryIndexes')) {
            if (this.flightStepRQ.condition.filter.itineraryIndexes.length === 1) {
                this.flightStepRQ.condition.filter.itineraryIndexes = this.flightStepRQ.condition.filter.itineraryIndexes.concat(Number(this.selectedItem.itinerary.itineraryIndex));
            }
        } else {
